...
the milestone rates the evaluation oracle tracks. Compare those against the
baseline in ``docs/DPPO_CLOSED_LOOP_STALL_HANDOFF.md`` (cube moved, cube lifted,
placed within 6 cm). Small-sample noise is large -- that document measured the
same checkpoint at 11/33 and 24/33 on two different scene sets -- so read this as
"does the policy still act on the cube", not as a success-rate measurement.
