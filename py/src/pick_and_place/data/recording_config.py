...
class SceneDraw:
    ...
    #: How simulated overhead sightings are produced. ``geometric`` is the
    #: dataset default: segmentation gates visibility while controlled beliefs
    #: supply poses. ``detector`` exercises the real optical pipeline.
    overhead_perception: str = "geometric"
    ...
    #: ``domain_randomization`` too: the light, the camera response, the sensor
    #: noise and the cube's resting orientation still have to be drawn from an
    #: envelope, and the sidecar names them rather than carrying them.
    frozen_rig: Path | None = None
