...
The trajectory speaks the *sim frame* (arm joints in radians, gripper a joint
angle in radians); a physical SO-101 follower speaks the *real frame* (arm joints
in degrees, gripper a 0-100 position). The conversion is:
...
Also here: packing a joint 6-vector into the dict shape lerobot's ``send_action``
takes, and reading one back out. None of it needs lerobot or any hardware —
:mod:`pick_and_place.hardware.follower` constructs the devices.
"""
...
def load_joint_zero_offsets(path: Path | str) -> dict[str, float]:
